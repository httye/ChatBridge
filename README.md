# 🌟 ChatBridge - 多服务器聊天互通插件

<div align="center">

**✨ 让你的多个服务器连接起来，实现畅聊无阻！** ✨

[![Version](https://img.shields.io/badge/version-1.0.0-blue.svg)](https://github.com/httye/ChatBridge)
...
|--------|------|
| `enabled` | 是否启用服务器前缀 |
| `format` | 前缀格式，`{server}` 会替换为服务器显示名称 |
| `position` | 前缀位置 |
...
编译后的 JAR 文件位于 `target/chat-bridge-1.0.0.jar`
...
- 设置 `format` 自定义前缀格式，使用 `{server}` 代表服务器名称
- 设置 `position` 控制前缀位置
...
| [Vault](https://www.spigotmc.org/resources/vault.34315/) | 🛡️ 提供玩家前缀/后缀（备选） |
